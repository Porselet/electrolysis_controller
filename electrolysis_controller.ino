...
static void handle_button(enum Button b)
{
    ...
    case BTN_UP:
        if (cursor == 0)
        {
            ...
            if (g_fill_threshold > FILL_MAX_ATM)
                g_fill_threshold = FILL_MAX_ATM;
            settings_mark_dirty();
        }
        else
        {
            uint32_t mins = g_hold_time_sec / 60UL + HOLD_STEP_MIN;
            if (mins > HOLD_MAX_MIN)
                mins = HOLD_MAX_MIN;
    ...
    case BTN_DOWN:
        ...
        {
            g_fill_threshold -= FILL_STEP_ATM;
            if (g_fill_threshold < FILL_MIN_ATM)
                g_fill_threshold = FILL_MIN_ATM;
            settings_mark_dirty();
        }
        else
        {
...
void loop()
{
    ...
    // TODO: временно передаём p1 дважды — на руках только один датчик.
    // Вернуть &p1, &p2 и проверить FAULT_MISMATCH, когда появится второй.
